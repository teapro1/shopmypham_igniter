...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='STORE MỸ PHẨM - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        $todaylimit = date("%Y-%m-%d", $todaylimit);
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->from($result[0]['mail_smtp'], 'STORE MỸ PHẨM');
            $this->email->to($email);
            $this->email->subject('Hệ thống Mỹ Phẩm - Quà Tặng Thành Viên Mới');
            $this->email->message('Bạn đã trở thành thành viên mới của STORE MỸ PHẨM, Cửa hàng tặng bạn 1 mã giảm giá 100.000đ : '.$tempcoupon.'<br>Mã này có giá trị tới '.$tempdatelimit.'<br>Hãy mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!! <br>Cảm ơn quý khách đã ghé thăm cửa hàng!');
            $this->email->send();
            $this->data['success']='Đăng ký thành công! Bạn đã nhận được 1 mã giảm giá cho thành viên mới, vui lòng kiểm tra email !!';

        }  
        $this->data['title']='STORE MỸ PHẨM - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('tratrinh19623@gmail.com', 'STORE MỸ PHẨM');
            $this->email->to($list['email']);
            $this->email->subject('Hệ thống Mỹ Phẩm - Lấy lại mật khẩu');
        ...
        $this->data['title']='STORE MỸ PHẨM - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='STORE MỸ PHẨM - Cập nhật mật khẩu mới';
