...
class Orders extends CI_Controller {
  ...
  public function status($id)
  {
    ...
    {
      $this->session->set_flashdata('error', 'Đơn hàng '.$row['orderCode'].' đã giao và thanh toán, không thể chỉnh sửa !');
      redirect('admin/orders','refresh');
    }
  ...
  public function trash($id)
  {
    ...
    if($status == 1){
      ...
      $this->Morders->orders_update($mydata, $id);
      $this->session->set_flashdata('success', 'Chuyển đơn hàng '.$row['orderCode'].' vào thùng rác thành công');
      redirect('admin/orders','refresh');
    }
  }
