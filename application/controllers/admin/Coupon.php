...
class Coupon extends CI_Controller {
	...
	public function insert()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_flashdata('success', 'Thêm mã giảm giá '.$_POST['code'].' thành công. Thời gian: '.$today.'');
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_flashdata('success', 'Cập nhật mã giảm giá '.$_POST['code'].' thành công');
			redirect('admin/coupon/','refresh');
		} 
