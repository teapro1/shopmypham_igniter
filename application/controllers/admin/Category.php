...
class Category extends CI_Controller {
	...
	public function insert()
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_flashdata('success', 'Thêm danh mục '.$_POST['parentid'].' thành công. Thời gian: '.$today.'');
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_flashdata('success', 'Cập nhật danh mục '.$_POST['name'].' thành công. Thời gian: '.$today.'');
			redirect('admin/category','refresh');
		} 
