      ...
      redirect('admin/E403/index','refresh');
		...
		if ($this->form_validation->run() == TRUE){
	        ...
	        //Lưu nhóm hình ảnh chi tiết
              ...
              $this->session->set_flashdata('success', 'Thêm Sản Phẩm '.$_POST['name'].' Thành Công. Thời Gian: '.$today.'');
    ...
    public function update($id){
         ...
         if ($this->form_validation->run() == TRUE){
          $mydata= array(
           ...
           'number' => $_POST['number']
              ...
              $this->session->set_flashdata('success', 'Cập Nhật Sản Phẩm '.$_POST['name'].' Thành Công. Thời Gian: '.$today.'');
              redirect('admin/product','refresh');
            } 
       ...
       public function import($id){
         ...
         if ($this->form_validation->run() == TRUE){
          ...
          $this->Mproduct->product_update($mydata, $id);
          $this->session->set_flashdata('success', 'Đã Nhập Thêm '.$_POST['number'].' Sản Phẩm ' .$row['name'].' Thành Công.'.' Số Lượng Sản Phẩm Còn Lại Trong Kho Là: '.$row['number']+$_POST['number']-$row['number_buy'].'. Thời Gian Nhập: '.$today);
          redirect('admin/product','refresh');
        }
        $this->data['row']=$row;
